...
def create_sklearn_modelPAC(random_state, validation_split):
...
def create_sklearn_modelMLP(random_state, validation_split):
    VALIDATION_SPLIT=validation_split
...
def create_modelMLP1(tokenizer,num_classes):
    NCLASSI=num_classes
...
def create_modelMLP2(tokenizer,num_classes):
    NCLASSI=num_classes
    vocab_size = len(tokenizer.word_index)+1   
    model = Sequential()
    #model.add(Dense(128, input_shape=(max_words,)))
...
def create_keras_modelMLP(X_train,y_train,X_test,y_test,model,num_classes,basedir,validation_split,k=1):
    BASE_DIR=basedir
    VALIDATION_SPLIT=validation_split
    BATCH=128
    EPOCHS=40
...
def setEmbeddingMatrix(embedding, word_index, EMBEDDING_DIM):
    #embedding matrix
...
def create_keras_modelCNN(raw_embedding,tokenizer,num_classes):
    EMBEDDING_DIM=300
    MAX_SEQUENCE_LENGTH=1500
    NCLASSI=num_classes
    embedding_matrix=setEmbeddingMatrix(raw_embedding,tokenizer.word_index, EMBEDDING_DIM)
    print('Training model.')
    # define model,  train a 1D convnet
    sequence_input = Input(shape=(MAX_SEQUENCE_LENGTH,), dtype='int32')
    embedded_sequences = Embedding(sequence_input, EMBEDDING_DIM, weights=[embedding_matrix], input_length=MAX_SEQUENCE_LENGTH, trainable=False)
